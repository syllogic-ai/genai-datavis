...
import { usePublicDashboardTheme } from "@/components/theme/PublicDashboardThemeProvider";

export function useDashboardChartColorsCompat() {
  // Try to get theme from either provider
  let getThemeStyles: (() => any) | null = null;
  
  try {
    const { getThemeStyles: regularGetThemeStyles } = useDashboardTheme();
    getThemeStyles = regularGetThemeStyles;
  } catch {
    // Not in regular dashboard theme provider, try public one
    try {
      const { getThemeStyles: publicGetThemeStyles } = usePublicDashboardTheme();
      getThemeStyles = publicGetThemeStyles;
    } catch {
      // Neither provider available, use null
      getThemeStyles = null;
    }
  }

  const styles = getThemeStyles?.() || null;
