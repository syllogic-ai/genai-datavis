...
async function setupJobsTable() {
  ...
  try {
    ...
    // Create the jobs table with simplified schema
    ...
    const addConstraintsQuery = `
      ...
      ALTER TABLE "jobs" ADD CONSTRAINT "jobs_dashboard_id_dashboards_id_fk" 
        FOREIGN KEY ("dashboard_id") REFERENCES "public"."dashboards"("id") ON DELETE no action ON UPDATE no action;
    `;
    ...
    console.error("❌ Error setting up jobs table:", error);
    throw error;
  } finally {
    await client.end();
  }
...
// Run the script
setupJobsTable().catch(console.error);