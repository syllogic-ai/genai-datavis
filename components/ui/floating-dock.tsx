...
const FloatingDockDesktop = ({
  ...
  return (
    <motion.div
      ...
      className={cn(
        "mx-auto hidden h-16 items-end gap-4 rounded-2xl bg-gray-50 px-4 pb-3 md:flex dark:bg-neutral-900 shadow-lg border border-gray-200 dark:border-neutral-800",
        className,
      )}
    >
      {items.map((item) => (
...
function IconContainer({
  ...
  return (
    <button onClick={handleClick} className="focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50 rounded-full">
      <motion.div
        ref={ref}
        style={{ width, height }}
        ...
        <AnimatePresence>
          {hovered && (
            <motion.div
              ...
              className="absolute -top-8 left-1/2 w-fit rounded-md border border-gray-200 bg-gray-100 px-2 py-0.5 text-xs whitespace-pre text-neutral-700 dark:border-neutral-900 dark:bg-neutral-800 dark:text-white z-50"
            >
              {title}
            </motion.div>
          )}
