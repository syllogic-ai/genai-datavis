...
export function ThemeSelector({ dashboardId }: ThemeSelectorProps) {
  ...
  return (
    <Select
      ...
      disabled={isLoading}
    >
      <SelectTrigger className="h-fit py-1 px-4 rounded-lg text-sm font-medium gap-2 border bg-background shadow-xs hover:bg-accent hover:text-accent-foreground dark:bg-input/30 dark:border-input dark:hover:bg-input/50">
                  ...
                  {[1, 2, 3].map((i) => (
                    <div
                      ...
                      style={{
                        backgroundColor:
                          theme.styles?.light?.[
                            `chart-${i}` as keyof typeof theme.styles.light
                          ] || "#000",
                      }}
                    />
                  ))}
